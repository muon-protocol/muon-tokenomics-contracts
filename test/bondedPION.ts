...
describe("bonPION", function () {
  ...
  beforeEach(async () => {
    ...
    BOOSTER_ROLE = await bonPion.BOOSTER_ROLE();
  ...
  describe("Add Boosted Balance", async function () {
    it("should allow booster to add boosted balance", async function () {
      const tokenId = await bonPion.callStatic.mint(user.address);
      await bonPion.mint(user.address);
      expect(tokenId).eq(1);
      expect(await bonPion.ownerOf(tokenId)).eq(user.address);
      ...
      await bonPion.connect(booster).addBoostedBalance(tokenId, amount);

      expect(await bonPion.boostedBalance(tokenId)).to.be.equal(amount);
    });

    it("should not allow non-booster to add boosted balance", async function () {
      const tokenId = await bonPion.callStatic.mint(user.address);
      const revertMSG = `AccessControl: account ${user.address.toLowerCase()} is missing role ${BOOSTER_ROLE}`;
      await bonPion.mint(user.address);
      expect(tokenId).eq(1);
      expect(await bonPion.ownerOf(tokenId)).eq(user.address);
